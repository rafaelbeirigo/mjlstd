...
class TestMjlstdMjlstdYsHist(unittest.TestCase):
    def setUp(self):
        from test_cases import eye_one_constants_mjlstd_Ys_hist1 as cf
        from test_cases import eye_one_parameters_mjlstd_Ys_hist1 as pf
    ...
    def test_mjlstd_Ys_hist(self):
        (_, _, Ys_hist) = mjlstd.mjlstd(p=self.params_obj, m=self.mjls_obj)
...
class TestMjlstdMjlstdYsHist4(TestMjlstdMjlstdYsHist):
    def setUp(self):
        from test_cases import eye_one_constants_mjlstd_Ys_hist4 as cf
        from test_cases import eye_one_parameters_mjlstd_Ys_hist4 as pf
...
class TestMjlstdMjlstdEyeTwo(TestMjlstdMjlstd):
    def setUp(self):
        # The (c)onstants (f)ile
        self.cf = eye_two_constants_mjlstd
        args = {'N': self.cf.N,
                'm': self.cf.m,
                'n': self.cf.n,
                'A': self.cf.A,
        ...
        # The (p)arameters (f)ile
        self.pf = eye_two_parameters_mjlstd
        args_p = {'L': self.pf.L,
                  'T': self.pf.T,
                  'K': self.pf.K,
                  'lambda_': self.pf.lambda_,
